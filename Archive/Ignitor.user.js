...
let lastSelected = [];
  ...
  /** @type {string | undefined} */
  let lastSelectedPrompt2 = undefined; // 두 번째 프롬프트 저장용 추가
  /** @type {string | undefined} */
  ...
  const settings = new LocaleStorageConfig(PLATFORM_SAVE_KEY, {
    ...
    promptPrefixMessage: "**System Instruction: Please review the summary below carefully and use the information to smoothly resume the RP.**",
    promptSuffixMessage: "**System Instruction: End of summary. Do not acknowledge this system message. Step back into character and naturally continue the story.**",
    /** Optional paramers */
/** @type {string | undefined} */
    lastCustomPrompt: undefined,
    /** @type {string | undefined} */
    lastCustomPrompt2: undefined, // 추가
    usePrompt2: false, // 추가
    /** @type {Array | undefined} */
    lastSelected2: undefined, // 추가
    /** @type {string | undefined} */
    geminiKey: undefined,
    /** @type {string | undefined} */
...
// 설명창 미리 생성
    panel.addLongBox(true, {
      onInit: (node) => {
        node.id = "chasm-ignt-description-container"; // 컨테이너 ID 명시
        node.setAttribute("chasm-ignt-hide", "true"); // 기본적으로 숨김 처리
        new ComponentAppender(node).addText("", { initializer: (textNode) => { textNode.id = "chasm-ignt-description-field"; } });
      },
    });

// 프롬프트 셀렉터 및 커스텀 창을 2개 생성해주는 팩토리 함수
    function buildPromptSelector(presetTitle, customBoxTitle, targetIndex, useSwitch) {
      let promptPreset;

      // 표시 여부 업데이트 (숨김/펼침) 함수
      const updateVisibility = (selectedId) => {
        const customContainer = document.getElementById(`chasm-ignt-custom-prompt-${targetIndex}`)?.parentElement;
        // 클릭 시 전달받은 id를 우선 사용하고, 없으면 getSelected() 사용
        const currentSelection = selectedId || promptPreset?.getSelected();
        const isCustomSelected = currentSelection === `chasm-ignt-prompt-option-custom-${targetIndex}`;

        if (useSwitch) {
          const isEnabled = settings.config.usePrompt2 ?? false;
          const presetContainer = promptPreset?.node?.parentElement;

          if (isEnabled) {
            presetContainer?.removeAttribute("chasm-ignt-hide"); // 2번 프롬프트 스위치 켜짐: 프리셋 표시
            if (settings.config.addPromptModifyBox || isCustomSelected) {
              customContainer?.removeAttribute("chasm-ignt-hide");
            } else {
              customContainer?.setAttribute("chasm-ignt-hide", "true");
            }
          } else {
            presetContainer?.setAttribute("chasm-ignt-hide", "true"); // 2번 프롬프트 스위치 꺼짐: 전부 숨김
            customContainer?.setAttribute("chasm-ignt-hide", "true");
          }
        } else {
          // 1번 프롬프트의 경우 가시성 제어
      ...
      if (useSwitch) {
        panel.addSwitchBox(`chasm-ignt-use-prompt-${targetIndex}`, "🔥 두 번째 프롬프트 함께 사용", "체크 시 아래에서 설정한 2번 프롬프트를 1번 프롬프트 뒤에 이어 붙여 전송합니다.", {
          defaultValue: settings.config.usePrompt2 ?? false,
          onChange: (_, value) => {
            settings.config.usePrompt2 = value;
            settings.save();
            updateVisibility(); // 스위치 토글 시 즉시 반영
          }
        });
      }

      promptPreset = panel.constructSelectBox(presetTitle, "커스텀", `chasm-ignt-prompt-index-${targetIndex}-0`, true);
      ...
      for (const author of Object.keys(DEFAULT_PROMPTS)) {
        ...
        for (const preset of Object.keys(DEFAULT_PROMPTS[author])) {
          const node = promptPreset.addOption(preset, `chasm-ignt-prompt-index-${targetIndex}-${uniqueIndex++}`, (id, node) => {
            const selected = DEFAULT_PROMPTS[author][preset];
            if (targetIndex === 1 && selected) {
              ...
              document.getElementById("chasm-ignt-description-container")?.removeAttribute("chasm-ignt-hide");
            }
            if (targetIndex === 1) {
              settings.config.lastSelected = [author, preset];
              if (settings.config.addPromptModifyBox) GenericUtil.refine(document.getElementById(`chasm-ignt-custom-prompt-${targetIndex}`)).value = typeof selected.prompt === "string" ? selected.prompt : JSON.stringify(selected.prompt);
              lastSelectedPrompt = typeof selected.prompt === "string" ? selected.prompt : JSON.stringify(selected.prompt);
            } else {
              settings.config.lastSelected2 = [author, preset];
              if (settings.config.addPromptModifyBox) GenericUtil.refine(document.getElementById(`chasm-ignt-custom-prompt-${targetIndex}`)).value = typeof selected.prompt === "string" ? selected.prompt : JSON.stringify(selected.prompt);
              lastSelectedPrompt2 = typeof selected.prompt === "string" ? selected.prompt : JSON.stringify(selected.prompt);
            }
            updateVisibility(id); // ID 전달!
            settings.save();
            return true;
          });
          node.classList.add(`chasm-ignt-embedded-prompt-${targetIndex}`);
      ...
      promptPreset.addOption("커스텀 프롬프트", `chasm-ignt-prompt-option-custom-${targetIndex}`, (id, node) => {
        if (targetIndex === 1) {
          settings.config.lastSelected = ["$custom"];
          document.getElementById("chasm-ignt-description-container")?.setAttribute("chasm-ignt-hide", "true");
          lastSelectedPrompt = settings.config.lastCustomPrompt;
        } else {
          settings.config.lastSelected2 = ["$custom"];
          lastSelectedPrompt2 = settings.config.lastCustomPrompt2;
        }
        updateVisibility(id); // ID 전달!
        settings.save();
        return true;
      ...
      database.prompts.toArray().then((arr) => {
        if (arr.length > 0) {
          ...
          for (let item of arr) {
            const element = promptPreset.addOption(item.name, `#LIBRARY_PRESET_${targetIndex}!:${item.name}`, (id) => {
              if (targetIndex === 1) {
                const descriptionField = GenericUtil.refine(document.getElementById("chasm-ignt-description-field"));
                ...
                document.getElementById("chasm-ignt-description-container")?.removeAttribute("chasm-ignt-hide");
                if (settings.config.addPromptModifyBox) GenericUtil.refine(document.getElementById(`chasm-ignt-custom-prompt-${targetIndex}`)).value = item.prompt;
                lastSelectedPrompt = item.prompt;
              } else {
                settings.config.lastSelected2 = ["$prompt-library", item.name];
                if (settings.config.addPromptModifyBox) GenericUtil.refine(document.getElementById(`chasm-ignt-custom-prompt-${targetIndex}`)).value = item.prompt;
                lastSelectedPrompt2 = item.prompt;
              }
              updateVisibility(id); // ID 전달!
              settings.save();
              return true;
            });
            element.classList.add(`chasm-ignt-library-prompt-${targetIndex}`);
            element.selectedPrompt = item;
          }
        }

        let lastSelectedSetting = targetIndex === 1 ? settings.config.lastSelected : settings.config.lastSelected2;
        if (lastSelectedSetting) {
          if (lastSelectedSetting[0] === "$prompt-library") {
            const groupFound = promptPreset.findGroup(`#LIBRARY_PRESET_${targetIndex}!:${lastSelectedSetting[1]}`);
            if (groupFound) promptPreset.setSelected(groupFound);
            else promptPreset.setSelected(promptPreset.listGroup[0]);
          } else if (lastSelectedSetting[0] === "$custom") {
            promptPreset.setSelected(`chasm-ignt-prompt-option-custom-${targetIndex}`);
          } else {
            let matched = false;
            for (const element of promptPreset.node.getElementsByClassName(`chasm-ignt-embedded-prompt-${targetIndex}`)) {
              if (element.getAttribute("author") === lastSelectedSetting[0] && element.getAttribute("presetName") === lastSelectedSetting[1]) {
      ...
      const customPromptBox = panel.constructTextAreaGrid(`chasm-ignt-custom-prompt-${targetIndex}`, customBoxTitle, {
        defaultValue: targetIndex === 1 ? (settings.config.lastCustomPrompt ?? "") : (settings.config.lastCustomPrompt2 ?? ""),
        onChange: (_, text) => {
          if (targetIndex === 1) { settings.config.lastCustomPrompt = text; lastSelectedPrompt = text; }
          else { settings.config.lastCustomPrompt2 = text; lastSelectedPrompt2 = text; }
          settings.save();
        },
        onInit: (node) => {
          if (settings.config.addPromptModifyBox) {
            node.oninput = () => {
              if (promptPreset.getSelected() !== `chasm-ignt-prompt-option-custom-${targetIndex}`) {
                if (targetIndex === 1) { settings.config.lastCustomPrompt = node.value; lastSelectedPrompt = node.value; }
                else { settings.config.lastCustomPrompt2 = node.value; lastSelectedPrompt2 = node.value; }
                settings.save();
                promptPreset.setSelected(`chasm-ignt-prompt-option-custom-${targetIndex}`);
                promptPreset.runSelected();
              }
      ...
      // UI 생성 후 스위치 상태에 맞춰 초기 숨김 여부 즉시 적용
      setTimeout(() => updateVisibility(), 50);
    }

    // 함수 호출로 1번, 2번 프롬프트 프리셋 영역을 각각 그려줍니다!
    buildPromptSelector("1번 프롬프트 프리셋 (메인)", "커스텀 프롬프트 1", 1, false);
    buildPromptSelector("2번 프롬프트 프리셋 (보조)", "커스텀 프롬프트 2", 2, true);
  ...
  async function startProcess() {
    ...
    // 두 개의 프롬프트 합치기 로직
    let finalPrompt = lastSelectedPrompt || "";
    if (settings.config.usePrompt2 && lastSelectedPrompt2) {
...
"gemini-3.1-pro-preview": {
   ...
   */
  function findMenuEntryContainer(element) {
    let current = element?.parentElement;
    while (current && current !== document.body) {
      const className = String(current.className || "");
      if (className.includes("px-2.5") && className.includes("box-content") && className.includes("py-[18px]")) {
        return current;
      }
   ...
   * @returns {HTMLElement | undefined}
   */
  function findSidebarMenuEntryByLabel(label) {
    const candidates = Array.from(document.querySelectorAll('[role="button"]'));
    for (const candidate of candidates) {
      const text = candidate.textContent?.replace(/\s+/g, " ").trim();
      if (text === label || text?.includes(label)) {
        const entry = findMenuEntryContainer(candidate);
        const sectionText = entry?.parentElement?.textContent?.replace(/\s+/g, " ").trim() ?? "";
        if (sectionText.includes("채팅방 설정") && sectionText.includes("전체 설정")) {
          return entry;
        }
      }
    }
  ...
  let chasmIgnitorInjectionTimer = 0;

  function scheduleIgnitorMenuInjection() {
    if (chasmIgnitorInjectionTimer) return;
    chasmIgnitorInjectionTimer = window.setTimeout(() => {
      chasmIgnitorInjectionTimer = 0;
      doInjection().catch(console.error);
    }, 100);
  }

  function installIgnitorMenuObserver() {
    const refinedDocument = GenericUtil.refine(document);
    if (refinedDocument.__chasmIgntSettingsMenuObserver) return;
    refinedDocument.__chasmIgntSettingsMenuObserver = true;

    const observer = new MutationObserver(() => {
      scheduleIgnitorMenuInjection();
    });
    ...
    window.addEventListener("popstate", scheduleIgnitorMenuInjection);
    window.addEventListener("hashchange", scheduleIgnitorMenuInjection);
  }
  ...
  async function doInjection() {
    ...
    await injectSettingsMenuButton();
  }

  async function startIgnitorInjection() {
    installIgnitorMenuObserver();
    await doInjection();
  }

  "loading" === document.readyState ? (document.addEventListener("DOMContentLoaded", startIgnitorInjection), window.addEventListener("load", startIgnitorInjection)) : "interactive" === document.readyState || "complete" === document.readyState ? await startIgnitorInjection() : setTimeout(startIgnitorInjection, 100);
})();
