# Crack UI Max 가이드
<img width="1920" height="98" alt="image" src="https://github.com/user-attachments/assets/6c9c71bc-a32e-49a5-aea2-614f0a4af410" />

**➢ 설정창** : PC / 태블릿 / 모바일 모두 검색창 옆 톱니바퀴를 눌러 사용 가능

**➢ Passive** : 모델 선택창에서 모델 관련 설명 문구가 사라짐  
<img width="477" height="84" alt="image" src="https://github.com/user-attachments/assets/156f1235-2f19-4c03-8b22-f32f3d3b8979" />

**➢ 테마 변경**  
<img width="1220" height="71" alt="image" src="https://github.com/user-attachments/assets/e5dbee21-0950-477c-93c4-09a2d764f86e" />  
⤷ 색상 : 라이트 모드 / 다크 모드 변경  
⤷ 작품 UI 변경 : 소설형 UI / 채팅형 UI 변경

***
### 채팅
***
<img width="1221" height="690" alt="image" src="https://github.com/user-attachments/assets/66cebeb9-424f-4602-b938-cad687c03b41" />

**➢ 대화창 폭 조절** : 보여지는 채팅 로그와 입력창의 폭을 조절  
⤷ PC / 태블릿 지원 ⭕  
⤷ 모바일 미지원 ❌

**➢ 이미지 사이즈 조절** : 채팅창 내 삽입되는 모든 이미지의 크기를 조절

**➢ 입력창 모델 변경 버튼** : 입력창 전송 버튼 옆에 모델 선택 버튼이 생김  
<img width="423" height="210" alt="image" src="https://github.com/user-attachments/assets/041fcc54-d58e-4d03-8eb4-93ee2cdea712" /><img width="510" height="223" alt="image" src="https://github.com/user-attachments/assets/064d9c91-cd5b-4cc7-bdd1-06cd8ff8ac22" />  
⤷ 표시할 모델 : 미선택한 모델은 상단 모델 선택창과 입력창 모델 선택창에서 모두 숨김

**➢ 채팅방 설정 자동 숨김** : 채팅방 설정창을 자동으로 숨김  
<img width="582" height="174" alt="image" src="https://github.com/user-attachments/assets/41c89866-fb3d-45a5-99cb-9950284d11e0" />  
⤷ PC : 화면 오른쪽 끝에 마우스를 가져가면 나타남  
⤷ 태블릿 / 모바일 : 오른쪽 핸들이나 우에서 좌로 슬라이드 기능으로 열 수 있음

**➢ 스탯창 숨김** : 스탯이 존재하는 채팅에서 상단 스탯창을 숨김

**➢ 스토리 자동 재생 끄기** : 입력창이 비어 있을 때 전송 버튼과 Enter 전송을 막음

**➢ 상황 이미지 끄기** : 세이프티 작품 좌측 하단의 상황 이미지 버튼을 숨김

**➢ 줄바꿈 최적화** : 앱과 동일하게 단어 단위로 줄바꿈

**➢ 읽는 중 자동 스크롤 방지** : 이전 로그를 읽는 중 출력이 완료되어도 맨 아래로 강제 이동하지 않음

**➢ 소설형 UI 모델 표기** : 소설형 UI 응답 좌측 하단에 사용된 모델 아이콘을 표시  
<img width="482" height="284" alt="image" src="https://github.com/user-attachments/assets/cf571a31-6803-49f1-8d63-4b6187bb0f6e" />  
⤷ 모델을 찾지 못하면 ? 아이콘이 표시됨  
⤷ 아이콘을 누르면 모델을 직접 선택하거나 자동 판정으로 되돌릴 수 있음

***
### 화면
***
**➢ 상단바 자동 숨김** : Crack 상단바를 자동으로 숨김  
<img width="850" height="92" alt="image" src="https://github.com/user-attachments/assets/e54840e2-ac5f-4d7c-a162-79a43aadd965" />  
⤷ PC : 화면 맨 위에 마우스를 가져가면 나타남  
⤷ 태블릿 / 모바일 : 화면 위쪽 가운데 핸들을 누르면 나타남  

**➢ 전체화면 버튼** : 채팅 화면에 전체화면 전환 버튼을 추가  
<img width="260" height="76" alt="image" src="https://github.com/user-attachments/assets/7b6386af-6fcf-43cf-9c76-e3c2640d2f4d" />  
⤷ iOS 지원 ❌  

**➢ 채팅 목록 자동 숨김** : 왼쪽 채팅 목록을 자동으로 숨김  
<img width="582" height="174" alt="image" src="https://github.com/user-attachments/assets/41c89866-fb3d-45a5-99cb-9950284d11e0" />  
⤷ PC : 화면 왼쪽 끝에 마우스를 가져가면 나타남  
⤷ 모바일 : 왼쪽 핸들이나 슬라이드 기능으로 열 수 있음  
⤷ 태블릿 : 슬라이드 기능으로 열 수 있음(태블릿은 핸들 공식 지원임)  

**➢ 썸네일 움짤 정지** : 움직이는 GIF 썸네일을 정지  
***
### 폰트
***
<img width="1199" height="660" alt="image" src="https://github.com/user-attachments/assets/704bab76-8d74-4857-9342-475a251648de" />  

**➢ 폰트 사용** : 폰트와 글자 꾸미기 기능 전체를 켜거나 끔  

**➢ 대사** : 지정된 따옴표 안의 대사 글꼴과 색상을 변경  
    ⤷ 감지 문자에서 대사로 인식할 여는 문자 / 닫는 문자를 추가하거나 삭제 가능  

**➢ 기본** : 일반 본문 글자의 글꼴과 색상을 변경  

**➢ 생각** : 작은따옴표 안의 생각 문장을 꾸밈  

**➢ 묘사** : 기울임체로 표시되는 묘사 문장을 꾸밈  
⤷ 기울임 효과만 따로 켜거나 끌 수 있음  

**➢ 굵게** : 굵은 글씨로 표시되는 문장을 꾸밈  

**➢ 코드블럭** : 코드블럭의 글자색, 배경색, 테두리와 불투명도를 조절  

**➢ 글자색** : 각 문장 종류의 글자색을 변경  
⤷ ↺ 버튼을 누르면 Crack 기본 색상으로 복구  

**➢ 배경색** : 각 문장 종류의 글자 뒤에 배경색을 적용  
⤷ 색상 옆 작은 스위치를 켜야 실제로 적용됨  

<img width="1199" height="577" alt="image" src="https://github.com/user-attachments/assets/ea988404-b653-454d-adfd-936ae517cb75" />  

**➢ 글자 모양** : 글씨 크기, 코드 글씨 크기, 두께, 행간, 자간, 문단 간격을 조절  

<img width="1198" height="751" alt="image" src="https://github.com/user-attachments/assets/1c8193d9-b7d7-4ab5-9a88-a9becb95b04e" />  

**➢ 글자 그림자** : 검정색 또는 흰색 글자 그림자를 적용  

**➢ 폰트 등록** : 웹폰트 주소나 기기의 TTF / OTF / WOFF / WOFF2 파일을 저장  

**➢ 폰트 선택** : 저장한 폰트를 본문, 코드블럭, 상단 타이틀에 각각 지정  

**➢ 폰트 설정 초기화** : 색상과 글자 모양 설정을 기본값으로 초기화  
⤷ 저장된 폰트 파일과 웹폰트 목록은 삭제되지 않음  

<img width="697" height="324" alt="image" src="https://github.com/user-attachments/assets/36d83552-817d-4b8c-9c16-05adfe5ff533" />  

**➢ 폰트 프리셋** : 현재 폰트, 색상, 글자 모양 설정을 이름을 정해 저장하고 불러옴  
***
### 배경
***
<img width="1219" height="420" alt="image" src="https://github.com/user-attachments/assets/19569cb2-d067-4a4e-b146-bf1b497d8b7c" />  

**➢ 배경색** : 채팅 화면의 배경색을 원하는 색으로 변경  

**➢ 배경 이미지** : 기기에서 선택한 이미지를 채팅 화면 배경으로 적용  
⤷ 배경색과 배경 이미지는 둘 중 하나만 사용 가능  
⤷ 이미지는 화면을 가득 채우는 방식으로 표시  

<img width="1326" height="369" alt="image" src="https://github.com/user-attachments/assets/ee2e655e-fcf1-46b7-aa1b-d5226cdd3f78" />  

**➢ 소설형 본문 배경** : 소설형 UI의 본문 뒤에 별도의 반투명 배경을 추가  
⤷ 본문 배경색과 투명도를 조절 가능  
⤷ 소설형 UI에서만 적용  
***
### 기타
***
**➢ 설정 저장** : 대부분의 설정은 현재 브라우저에 자동 저장됨

**➢ 폰트 파일 / 배경 이미지** : 외부 서버에 업로드되지 않고 현재 브라우저 내부에 저장됨

**➢ 업데이트 후 이상이 있을 경우** : 기존 Crack 탭을 닫고 새 탭에서 다시 실행 또는 **확프 끄고 새탭으로 다시 켜기**

**➢ Plus와 Max 또는 이전 버전을 동시에 실행하면 기능이 충돌할 수 있으므로 하나만 사용**
