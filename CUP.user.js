...
(() => {
  ...
  function addStyle() {
    const css = `
      ...
      /*
       * 대화창 폭 조절
       * - 태블릿/PC 폭(768px 이상)에서만 작동하도록 미디어 쿼리로 격리
       * - 좁은 휴대폰 폭(767px 이하)에서는 타 확장프로그램(턴수, 라존데) 레이아웃 붕괴 방지를 위해 순정 폭 유지
       */
  ...
  function isChatWidthSupportedViewport() {
    // 터치 여부로 막으면 태블릿까지 같이 막힌다.
    // 폭 조절은 라디오존데/턴수/계산기 충돌이 나는 좁은 휴대폰 폭만 제외한다.
  ...
  function updateChatWidthUi() {
    ...
    if (slider) {
      ...
      slider.title = supported ? '' : '휴대폰 폭에서는 비활성화됨';
    }

    if (value) {
      value.textContent = supported ? formatChatWidthDisplay(chatWidthPercent) : '휴대폰 꺼짐';
    }
  }
        ...
        <div class="crack-ui-section" data-crack-ui-section="chat" data-open="${chatSectionOpen ? '1' : '0'}">
          ...
          <div class="crack-ui-section-body" data-crack-ui-section-body="chat">
            ...
            <div class="crack-ui-range-row">
              ...
              <div class="crack-ui-row-desc">
                태블릿/PC 폭에서만 적용. 휴대폰 폭에서는 라존데·턴수 충돌 방지로 꺼짐
              </div>
